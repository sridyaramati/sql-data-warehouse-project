...
BEGIN
	...
	BEGIN TRY
		...
		-- CRM_CUST_INFO table:
		-- Removing duplicates from primary key: cst_id, trimming unwanted spaces for strings, data standardization for abbrevations
		...
		* ,
		...
		prd_start_dt,
		DATEADD(day, -1, LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)) AS prd_end_dt -- calculate as one day before next start date
		...
		END AS sls_order_dt, -- handling invalid data, data type casting
		...
		CASE WHEN sls_sales IS NULL OR sls_sales <=0 OR sls_sales != sls_quantity * ABS(sls_price)
			...
			ELSE sls_sales
		END AS sls_sales, -- recalcualte sales if original value is missing or incorrect
		sls_quantity,
